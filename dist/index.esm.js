import o, { useRef, useState, useCallback, useEffect, useMemo, useDebugValue, createElement, useContext, createContext } from 'react';
import { Stage, Layer, Image as Image$1, Shape } from 'react-konva';
...
const useFrameSelection = ({ onSelected, showBorderAfterDrag = false, borderClassName, borderStyle }) => {
    ...
    const SelectionBorder = useMemo(() => (jsxRuntimeExports.jsx("div", { className: borderClassName, style: Object.assign(Object.assign({ position: 'absolute', border: '2px solid #1890ff', backgroundColor: 'rgba(24, 144, 255, 0.1)', pointerEvents: 'none', zIndex: 999 }, borderStyle), borderPosition) })), [borderClassName, borderPosition, borderStyle]);
    useEffect(() => {
        return () => {
            var _a;
...
function useImageRender(imgUrl, setScale) {
    ...
    const computeScale = useCallback((imageSize, containerSize) => {
...
function useEditorContextInit(initialState) {
    ...
    // 统一管理编辑器状态
    const [state, setState] = useState(Object.assign({ mode: EditorMode.DrawLine, lineWidth: 20, scale: 0 }, initialState));
    // 状态更新方法
    const setMode = useCallback((mode) => {
        setState(prev => (Object.assign(Object.assign({}, prev), { mode })));
    }, []);
    const setLineWidth = useCallback((lineWidth) => {
...
const EditorToolsContext = createContext(null);
...
const EditorCore = (props) => {
    ...
    const createCanvas = useCallback((renderScale) => {
    ...
    const renderLine = useCallback((ctx, data, scale, reverse) => {
        ctx.lineCap = 'round';
        ctx.lineJoin = 'round';
    ...
    const renderMask = useCallback((records, renderScale) => {
    ...
    const [maskCanva, setMaskCanva] = useState(null);
    const [processedCanva, setProcessedCanva] = useState(null);
    const renderProcessedImage = useCallback(function (img, maskCanva, renderScale) {
    ...
    return (jsxRuntimeExports.jsx(EditorContainer, { ref: containerRef, children: img && imageSize.width > 0 && imageSize.height > 0 && (jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [jsxRuntimeExports.jsx(EditorPanel, { children: jsxRuntimeExports.jsxs(ImageContainer, { width: renderImageSize.width, height: renderImageSize.height, "$isViewMode": mode === EditorMode.View, style: Object.assign(Object.assign({}, drag.style), { cursor: CursorMod[mode] }), onMouseDown: onMouseDown, onMouseMove: onMouseMove, onMouseUp: onMouseUp, onMouseLeave: onMouseLeave, children: [isDrawLineMode(mode) && drawLine.brushCursor, frameSelection.SelectionBorder, jsxRuntimeExports.jsxs(Stage, { width: renderImageSize.width, height: renderImageSize.height, children: [jsxRuntimeExports.jsx(Layer, { children: jsxRuntimeExports.jsx(Image$1, { image: img, x: 0, y: 0, width: renderImageSize.width, height: renderImageSize.height }) }), jsxRuntimeExports.jsx(Layer, { opacity: maskOpacity, children: maskCanva && jsxRuntimeExports.jsx(Image$1, { image: maskCanva, x: 0, y: 0, width: renderImageSize.width, height: renderImageSize.height }) })] })] }) }), jsxRuntimeExports.jsx(EditorPanel, { children: jsxRuntimeExports.jsxs(ImageContainer, { width: renderImageSize.width, height: renderImageSize.height, "$isViewMode": mode === EditorMode.View, style: drag.style, onMouseDown: e => {
                            if (mode !== EditorMode.View)
                                return;
                            drag.handleDragStart(e);
                        }, children: [frameSelection.SelectionBorder, isDrawLineMode(mode) && drawLine.brushCursor, jsxRuntimeExports.jsxs(Stage, { width: renderImageSize.width, height: renderImageSize.height, children: [jsxRuntimeExports.jsx(Layer, { children: jsxRuntimeExports.jsx(Shape, { sceneFunc: ctx => {
                                                // 绘制网格背景，实现透明背景
                                                createTransparentGridPattern(ctx, renderImageSize);
                                            } }) }), jsxRuntimeExports.jsx(Layer, { children: processedCanva && jsxRuntimeExports.jsx(Image$1, { image: processedCanva, x: 0, y: 0, width: renderImageSize.width, height: renderImageSize.height }) })] })] }) })] })) }));
};
...
export { EditorContextProvider, EditorCore, EditorMode, HistoryAction, ToolControlPanel, CutoutEditor as default, useDragble, useEditorContext, useFrameSelection, useHistoryRecords, useMemoFn };
